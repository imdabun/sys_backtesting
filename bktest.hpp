...
namespace Backtest_env {
  ...
  class Alpha_Factor {
  // A meta-algorithm that assigns weights to a library of alphas based on incoming data
  private:
  ...
  public:
     ...
     *      initialize alpha_factor (taking params from BTSettings)
     ...
     *        call alpha_factor.forward(update_index) to get a cumulative alpha vector (alpha)
